...
def channel_config(chandle, status, enable, channels_, couplings_, cranges_, offsets_):
    for i in range(len(channels_)):
        indx = "setCh"+channels[channels_[i]]
        status[indx] = ps.ps2000aSetChannel(chandle, channels_[i], enable, couplings_[i], cranges_[i], offsets_[i])
